...
double erg_integrand_gagg(double erg, void * params)
{
  ...
  const double factor = 1.0e-4*pow(radius_sol/(1.0e-2*keV2cm),3) / (pow(distance_sol,2) * (1.0e6*hbar));
  ...
  double exposure = (p3->eff_exp->interpolate(erg))/(24.0*60.0*60.0);
  ...
  gsl_integration_qag (&f2, r_min, r_max, 0.1*ergint_abs_prec, 0.1*ergint_rel_prec, int_space_size, gagg_method, p3->w2, &spectral_flux, &spectral_flux_error);

  return 0.5*gsl_pow_2(erg/pi)*exposure*factor*spectral_flux*conversion_prob_factor*sincsq;
}
