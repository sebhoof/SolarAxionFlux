...
The code is written in C++ and contains Python files for data processing tasks. It only depends on the GSL library (v2.4 or higher) and, optionally, on Python v3.x to build Python wrappers with pybind11 (requires cython).
...
## Troubleshooting
Note that some fixes require to run `make clean` or deleting the contents of your `build/` directory in order to take effect/by recognised by the CMAKE system.
